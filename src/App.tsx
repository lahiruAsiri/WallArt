...
function LogPanel({ logs, setLogs }: { logs: string[]; setLogs: React.Dispatch<React.SetStateAction<string[]>> }) {
  ...
  return (
    <Html position={[0.5, 0.5, -0.3]}>
      <div className={`w-64 bg-gray-900 text-white p-4 rounded-lg shadow-lg transition-all duration-300`}>
        {visible ? (
          <>
...
function ControlPanel({ position, rotation, onPositionChange, onRotationChange }: {
  ...
  return (
    <Html position={[-0.5, 0.5, -0.3]}>
      <div className={`w-64 bg-gray-800 text-white p-4 rounded-lg shadow-lg transition-all duration-300`}>
        {isMinimized ? (
          <button
            className="w-full py-2 bg-blue-500 hover:bg-blue-600 rounded"
